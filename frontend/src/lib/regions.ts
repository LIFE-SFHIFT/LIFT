// 전국 시/도 → 시/군/구 목록.
// - 특별시·광역시는 자치구/군 전체를 담는다(서울 25개 자치구 등).
// - 도(道)는 시/군 단위로 담는다. 100만 이상 대도시의 행정구(수원 장안구 등)까지는
//   내려가지 않고 시 단위로 둔다(주소 입력 폼에서 통용되는 수준).
// - 세종특별자치시는 하위 자치구가 없어 자기 자신 한 개만 둔다.
// 2023년 7월 군위군의 대구 편입까지 반영.
...
/** 특정 시/도의 시·군·구 목록. 알 수 없는 시/도는 빈 배열. */
export function sigunguListOf(sido: string | null | undefined): string[] {
  if (!sido) return [];
  return REGIONS[sido] ?? [];
}
